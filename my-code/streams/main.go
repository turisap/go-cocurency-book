...
func wsMain() {
	...
	//ctx, cancel := requests.WithCancel(requests.Background())
	//defer cancel()
...
func httpMain() {
	...
	filteredEntries := filters.MinFilter(0.001, entries)
	rangeFiltered := filters.RangeFilter(filteredEntries, 0.99, 0.999)
	entryCh, errCh := filters.ErrFilter(rangeFiltered)
	resultCh := filters.MovingAvg(0.5, 5, entryCh)
